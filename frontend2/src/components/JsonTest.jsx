...
function Answers() {
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...studentDescAnswer];
        list[index][name] = value;
        setstudentDescAnswer(list);
    };

    const handleMcqAnswerChange = (e, i) => {
        const { name, value } = e.target;
        const list = [...studentMcqAnswer];
        list[i][name] = value;
        setstudentMcqAnswer(list);
    }
    ...
    const submitAnswer = (e) => {
        ...
        let answers = {
            ...
            "TypeOfExam": TestData.TypeOfExam,
            "Questions": studentDescAnswer,
            "Mcqs": studentMcqAnswer
        }
        console.log(answers);
        axios.post(`http://localhost:3001/answer`, answers).then(res => {
            console.log(res);
            history.goBack();
        }).catch(err => console.log(err))
    }
                            ...
                            mcqQuestion.map((question, i) => {
                                return (
                                    <div key={i}>
                                        ...
                                        <input className="form-check-input" type="radio" name="studentAnswer" value={question.Option1} id={question.Option1} onChange={(e) => handleMcqAnswerChange(e, i)} />
                                        <label className="form-check-label" htmlFor={question.Option1}> {question.Option1}</label> <br />
                                        <input className="form-check-input" type="radio" name="studentAnswer" value={question.Option2} id={question.Option2} onChange={(e) => handleMcqAnswerChange(e, i)} />
                                        <label className="form-check-label" htmlFor={question.Option2}> {question.Option2}</label> <br />
                                        <input className="form-check-input" type="radio" name="studentAnswer" value={question.Option3} id={question.Option3} onChange={(e) => handleMcqAnswerChange(e, i)} />
                                        <label className="form-check-label" htmlFor={question.Option3}> {question.Option3}</label> <br />
                                        <input className="form-check-input" type="radio" name="studentAnswer" value={question.Option4} id={question.Option4} onChange={(e) => handleMcqAnswerChange(e, i)} />
                        ...
                        }
                        {
                            DescriptionQurestion.map((x1, i) => {
                                return (
                                    <div key={i}>
                                        <h5>{x1.Question} </h5>
                                        <input type="text" name="studentAnswer" onChange={(e) => handleInputChange(e, i)} /> <br />
                                    </div>
                                );
                            })
                        }
